...
var App = (function() {
    ...
    function loadConfig() { API_BASE = localStorage.getItem('api_base') || 'https://api.redflix.com'; }
    
    function checkAuth() {
        var token = localStorage.getItem('token');
        accessCode = localStorage.getItem('access_code');
        if (token) validateToken(token);
        else if (accessCode) validateAccessCode(accessCode);
        else showLoginScreen();
    }
    
    function validateToken(token) {
        fetch(API_BASE + '/api/auth/me', { headers: { 'Authorization': 'Bearer ' + token } })
        .then(function(response) { if (response.ok) return response.json(); throw new Error('Invalid token'); })
        .then(function(data) { user = data; loadContent(); showHomeScreen(); })
        .catch(function(error) { console.error('Token validation failed:', error); localStorage.removeItem('token'); showLoginScreen(); });
    }
    
    function validateAccessCode(code) {
        fetch(API_BASE + '/api/access-codes/validate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code: code }) })
        .then(function(response) { if (response.ok) return response.json(); throw new Error('Invalid access code'); })
        .then(function(data) { if (data.valid) { loadContent(); showHomeScreen(); } else { localStorage.removeItem('access_code'); showLoginScreen(); } })
        .catch(function(error) { console.error('Access code validation failed:', error); localStorage.removeItem('access_code'); showLoginScreen(); });
    }
    
    function loadContent() {
        fetch(API_BASE + '/api/content?type=live')
        .then(function(response) { return response.json(); })
        .then(function(data) { channels = data.filter(function(item) { return item.isLive && item.channelNumber; }); channels.sort(function(a, b) { return a.channelNumber - b.channelNumber; }); })
        .catch(function(error) { console.error('Error loading channels:', error); });
        
        fetch(API_BASE + '/api/content')
        .then(function(response) { return response.json(); })
        .then(function(data) { content = data; renderContent(); })
        .catch(function(error) { console.error('Error loading content:', error); });
    }
    ...
    function renderContent() {
        ...
        var html = '<div class="main-container"><header class="header"><div class="logo">RedFlix</div><nav class="nav">';
        ...
        html += '</nav></header><main class="content-area" id="content-area">' + renderHomeContent() + '</main></div>';
        
        app.innerHTML = html;
        document.querySelectorAll('.nav-item').forEach(function(item) { item.addEventListener('click', function() { navigateToScreen(this.getAttribute('data-screen')); }); });
        Navigation.updateFocusableElements();
    }
    
    function renderHomeContent() {
        var html = '<section class="content-section"><h2>Continuar Assistindo</h2><div class="content-row" id="continue-watching"></div></section>';
        html += '<section class="content-section"><h2>Filmes Populares</h2><div class="content-row">';
        content.filter(function(item) { return item.type === 'movie'; }).slice(0, 10).forEach(function(movie) { html += renderContentCard(movie); });
        html += '</div></section><section class="content-section"><h2>Series Populares</h2><div class="content-row">';
        content.filter(function(item) { return item.type === 'series'; }).slice(0, 10).forEach(function(serie) { html += renderContentCard(serie); });
    ...
    function renderContentCard(item) {
        ...
        return '<div class="content-card" data-focusable="true" data-id="' + item.id + '" onclick="App.playContent(' + item.id + ')"><img src="' + posterUrl + '" alt="' + item.title + '"><div class="card-title">' + item.title + '</div></div>';
    }
    ...
    function renderChannelsContent() {
        ...
        channels.forEach(function(channel) {
            ...
            html += '<div class="channel-number">' + channel.channelNumber + '</div><div class="channel-name">' + channel.title + '</div></div>';
        });
        return html + '</div></section>';
    }
    ...
    function showSearchScreen() {
        var contentArea = document.getElementById('content-area');
        contentArea.innerHTML = '<section class="search-section"><h2>Buscar</h2><div class="search-container"><input type="text" id="search-input" data-focusable="true" placeholder="Digite para buscar..." class="search-input"></div><div class="search-results" id="search-results"></div></section>';
    ...
    function goToChannel(channelNumber) {
        ...
        if (channel) { currentChannel = channel; showPlayerScreen(); Player.play(channel.streamUrl); showChannelInfo(channel); }
        else showMessage('Canal ' + channelNumber + ' nao encontrado');
    }
    ...
    function showLoginScreen() {
        var app = document.getElementById('app');
        app.innerHTML = '<div class="login-screen"><div class="login-container"><h1>RedFlix</h1><p>Digite seu codigo de acesso</p><input type="text" id="access-code-input" data-focusable="true" placeholder="Codigo de acesso" class="login-input"><button data-focusable="true" onclick="App.submitAccessCode()" class="login-button">Entrar</button></div></div>';
        Navigation.updateFocusableElements();
    }
    
    function submitAccessCode() { var code = document.getElementById('access-code-input').value; if (code) { localStorage.setItem('access_code', code); validateAccessCode(code); } }
